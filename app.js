...
function renderMatches(){
  ...
  $("#matches").innerHTML=normal.map(m=>{
    ...
    const opponentLabel=escapeHtml(opponent?.display_name||"Compañero");
    return `<article class="match-card ${e8?"e8-active":""} ${matchResolved(m)?"has-result":"pre-match"} ${mp?.pick?"has-pick":"no-pick"}" data-resolved="${matchResolved(m)}" data-outcome="${outcome}" data-e8="${e8}">
      ...
      <div class="pick-row">${["1","X","2"].map(v=>{const mine=mp?.pick===v,other=reveal&&oppPick?.pick===v;return `<button class="pick ${mine?"selected":""} ${other?"opponent-selected":""}" data-match="${m.number}" data-pick="${v}" ${locked?"disabled":""} ${other?`title="Selección de ${opponentLabel}"`:""}>${mine?`<i class="pick-self-marker" aria-hidden="true"></i>`:""}${other?`<i class="pick-opponent-marker" aria-hidden="true"></i>`:""}<span>${v}</span><small>${v==="1"?"Local":v==="X"?"Empate":"Visitante"}</small></button>`}).join("")}</div>
